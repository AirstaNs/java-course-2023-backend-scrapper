...
public class HelpCommand extends AbstractCommand {
    ...
    public SendMessage handle(Update update) {
        ...
        commands.forEach(command -> message.append(StringSubstitutor.replace(format,
            Map.of("command", command.command(), "description", textResolver.resolve(command.description())),
            "%",
            "%"
        )));

...
public class TrackCommand extends AbstractCommand {
    ...
    public SendMessage handle(Update update) {
        ...
        if (link == null) {
            return new SendMessage(chatId,
                textResolver.resolve("command.track.invalid_command_usage", Map.of("command", command()))
            );
        }
        if (!isLinkValid(link)) {
    ...
    protected SendMessage createInvalidLinkMessage(Long chatId, String link) {
        return new SendMessage(chatId, textResolver.resolve("command.track.invalid_request", Map.of("link", link)));
    }
    ...
    public SendMessage performAction(Long chatId, String link) {
        ...
        return new SendMessage(chatId, textResolver.resolve("command.track.success", Map.of("tracking_link", link)));
    }
}

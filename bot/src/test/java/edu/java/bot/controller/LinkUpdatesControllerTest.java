...
public class LinkUpdatesControllerTest {
    ...
    public void handleUpdatesShouldWorkCorrectly() throws Exception {
        mockMvc.perform(MockMvcRequestBuilders.post("/updates").content("""
            {
              "id": 1,
        ...
        Mockito.verify(linkNotificationService)
               .notifyLinkUpdate(new LinkUpdate(1L, URI.create("https://example.com"), "string", List.of(0L)));
    }

    @Test
    public void handleUpdatesShouldReturnErrorWhenInputIncorrect() throws Exception {
        var result = mockMvc.perform(MockMvcRequestBuilders.post("/updates").content("""
            {
              "id": 1,
              "url": "https://example.com"
            }
        ...
        ApiErrorResponse error = objectMapper.readValue(result.getResponse().getContentAsString(),

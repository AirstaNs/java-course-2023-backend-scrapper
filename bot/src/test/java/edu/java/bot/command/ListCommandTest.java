...
public class ListCommandTest {
    ...
    public void handleShouldReturnEmptyListMessageWhenListIsEmpty() {
        ...
        Mockito.when(mockBotService.listLinks(Mockito.anyLong())).thenReturn(new ListLinksResponse(List.of()));
        ListCommand command = new ListCommand(createMockTextResolver(), mockBotService);
    ...
    public void handleShouldReturnListMessageWhenListIsNotEmpty() {
        ...
        Mockito.when(mockBotService.listLinks(Mockito.anyLong()))
               .thenReturn(new ListLinksResponse(List.of(new Link("link1", UUID.randomUUID()),
    ...
    private TextResolver createMockTextResolver() {
        ...
        Mockito.when(mockTextResolver.resolve(Mockito.eq("command.list.main"), Mockito.anyMap())).thenReturn("List:\n");
